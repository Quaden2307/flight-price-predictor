...
- **date-grouped** (GroupShuffleSplit on `origin+destination+departure_at`) — **the honest pass/fail gate.** Reference points: **0.166 = LR baseline (run #9)**; **0.146 = Tier-A XGB (run #11)**; ~0.133 was a *tuned native-cat XGB with richer features + Tier-B* (June 3 deep dive) — a model target, not a regime constant.
...
| 10 | 2026-07-13 | a4f3f2b +dirty | same as #9 (date-grouped gate) | 156987/40226 | **baseline-v1 Tier-A ONLY** (dropped `airline`, `airline_type`, `transfers` in `prepare_xy`) | LR (defaults) | 0.171 | 0.171 | **0.166–0.175** | **THE DEPLOYABLE BASELINE: 0.171.** First number measured under the full honest contract (date-grouped gate + query-time features only) — everything the product's backend will actually have. Tier-B cost for LR = **+0.005 vs #9, exactly at the tie threshold** — real but borderline, far under the feared 1–3pp; LR barely used airline/transfers. Expect a **larger** Tier-A penalty for stronger models (June XGB leaned on airline heavily). train = val again → no memorization. **Scoreboard work complete (#8 CI, #9 gate, #10 contract); every subsequent run is pure modeling, fighting 0.171 down.** Realistic strong-model zone: ~0.145–0.155 (data floor ~0.145; XGB's 0.133 was Tier-B-assisted). |
| 11 | 2026-07-13 | 6c94d2b +dirty | same as #9/#10 (date-grouped gate; rows identical → same frozen deal) | 156987/40226 | baseline-v1 Tier-A (identical matrix to #10 — `prepare_xy` imported from train_lr) | **XGBRegressor** (300 trees, depth 6, lr 0.1, subsample 0.8, colsample_bytree 0.8, seed 42) | 0.129 | **0.146** | **0.142–0.150** | **First modeling win: 0.171 → 0.146 (-0.025, ~5× the tie threshold).** Untuned tree lands at the top of the predicted strong-model zone, ≈ the ~0.145 data-floor estimate — most model headroom closed in one move; remaining levers now fight for thousandths. train 0.129 vs val 0.146 = mild memorization gap (subsample/colsample working). **Regime-sensitivity measured:** same model+features scored **0.205 on the retired chronological split** (stale pre-freeze train_xgb.py, accidental re-run) vs 0.146 here — trees can't extrapolate to far-future departures, LR can (#9 tie); cleanest evidence yet for the date-grouped gate. Caveat: hyperparams inherited from the old file, not chosen this run — recorded verbatim, revisit deliberately if tuning. |
...
- [x] **#10 — Tier-A-only run** on the date-grouped gate → **deployable baseline = 0.171** (CI 0.166–0.175). Tier-B cost LR only +0.005 (borderline tie). Scoreboard era done — modeling era starts at #11.
- [x] **#11 — XGBoost swap** (same split/features) → **val 0.146** (CI 0.142–0.150), beats LR by -0.025. Already ≈ the ~0.145 data floor; expect thousandths from here, not points.
...
### baseline-v1 Tier-A (active since run #10)
- Numeric: `distance_km`, `lead_time_days`, `is_international`, `route_mean_log_price`
- Dummified: `day_of_week`, `month_of_year`
- Target: `log_price` (i.e. `np.log(price)`)
- `prepare_xy` (train_lr.py) now hard-drops the Tier-B columns (`airline`, `airline_type`, `transfers`) — **all future runs are Tier-A by default**; features.py still builds Tier-B so a deliberate Tier-B comparison run stays a one-line change.

### baseline-v1 (runs #7–9, retired)
- As above plus Tier-B: `transfers` numeric; `airline`, `airline_type` dummified. Measured Tier-B value for LR: +0.005 (run #9 vs #10).
