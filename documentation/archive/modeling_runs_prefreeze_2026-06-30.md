> **ARCHIVED 2026-06-30.** Runs #1-6 below were logged on the PRE-FREEZE pipeline (drifting quantile split; evolving airport-coverage and feature understanding; before the 2026-06-11 deep dive). Kept for history only — they are NOT comparable to the current frozen-split sheet at ../modeling_runs.md. See that file and the memory note eval-regime-and-012-verdict for the current regime/Tier-A/floor findings.
...
| 6 | 2026-06-01 | 408d912 +dirty | 68315/14635 | baseline-v1 | XGB sweep — 18 cfg: depth{3,4,6}×mcw{1,5,20}×λ{1,10}, fixed n300/lr.1/ss.8/cs.8 | 0.122 (best) | **0.178 (best)** | Regularization sweep on full data (CSV → `experiments/run3_xgb_sweep.csv`). **0/18 configs beat the 0.166 LR bar**; best val 0.178 at depth=6/mcw=5/λ=10. Trend reversed vs run #3: deeper trees now win. ~~CONCLUSION: the signal is essentially linear~~ — **RETRACTED, see Note B below.** |

> **⎯⎯ Note A — Airport-coverage fix (2026-06-01, commit 408d912), applied between runs #3 and #4 ⎯⎯**
...
> **⎯⎯ Note B — ⚠ Deep audit (2026-06-01), after run #6: the LR>XGB verdict was a SPLIT ARTIFACT, not a real result ⎯⎯**
